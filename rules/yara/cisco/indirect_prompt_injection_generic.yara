...
rule indirect_prompt_injection_generic{
    ...
    meta:
        author = "Cisco"
        title = "Indirect Prompt Injection Detection"
        title_zh = "间接提示词注入检测"
        description = "Detects indirect prompt injection via instruction manipulation from external sources"
        description_zh = "检测来自外部来源并试图操纵指令的间接提示词注入"
        remediation = "Treat external content as untrusted data and isolate it from system or tool instructions."
        remediation_zh = "将外部内容视为不可信数据，并与系统或工具指令隔离。"
