...
rule code_execution_generic{
    ...
    meta:
        author = "Cisco"
        title = "Dangerous Code Execution Detection"
        title_zh = "危险代码执行检测"
        description = "Detects dangerous code execution patterns with untrusted input in agent skills"
        description_zh = "检测代理技能中使用不可信输入触发危险代码执行的模式"
        remediation = "Validate untrusted input and avoid passing user-controlled data into eval, exec, shell, or deserialization sinks."
        remediation_zh = "校验不可信输入，避免将用户可控数据传入 eval、exec、shell 或反序列化危险调用点。"
