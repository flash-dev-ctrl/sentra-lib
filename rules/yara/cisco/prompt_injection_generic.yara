...
rule prompt_injection_generic{
    ...
    meta:
        ...
        description_zh = "检测试图覆盖指令或强制恶意工具调用的提示词字符串"
        remediation = "Sanitize user-controlled prompts and enforce instruction hierarchy at tool boundaries."
        remediation_zh = "清理用户可控提示词，并在工具边界强制执行指令层级。"
