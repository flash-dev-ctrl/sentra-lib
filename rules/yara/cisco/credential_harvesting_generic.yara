...
rule credential_harvesting_generic{
    ...
    meta:
        ...
        description_zh = "检测 API Key、密码、令牌和证书等敏感信息暴露或凭据收集模式"
        remediation = "Remove exposed secrets, rotate affected credentials, and use a secure secret manager."
        remediation_zh = "移除暴露的密钥，轮换受影响凭据，并使用安全的凭据管理器。"
