...
rule autonomy_abuse_generic{
    ...
    meta:
        author = "Cisco"
        title = "Unbounded Autonomy Detection"
        title_zh = "无边界自主行为检测"
        description = "Detects unbounded autonomy patterns that bypass user control"
        description_zh = "检测绕过用户控制的无边界自主行为模式"
        remediation = "Review the instruction text and require explicit user confirmation for autonomous or irreversible actions."
        remediation_zh = "审查指令文本，并要求自主或不可逆操作必须经过用户明确确认。"
