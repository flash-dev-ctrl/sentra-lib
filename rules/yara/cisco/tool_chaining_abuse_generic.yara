...
rule tool_chaining_abuse_generic{
    ...
    meta:
        author = "Cisco"
        title = "Tool Chaining Abuse Detection"
        title_zh = "链式工具调用滥用检测"
        description = "Detects suspicious tool chaining patterns that could lead to data exfiltration"
        description_zh = "检测可能导致数据外传的可疑工具链调用模式"
        remediation = "Review chained tool calls and prevent sensitive data from flowing into untrusted tools."
        remediation_zh = "审查链式工具调用，防止敏感数据流入不可信工具。"
