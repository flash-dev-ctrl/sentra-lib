...
rule prompt_injection_unicode_steganography{
    ...
    meta:
        author = "Cisco"
        title = "Unicode Steganography Prompt Injection Detection"
        title_zh = "Unicode 隐写提示词注入检测"
        description = "Detects hidden Unicode characters used for invisible prompt injection and steganography"
        description_zh = "检测用于隐藏提示词注入和隐写的不可见 Unicode 字符"
        remediation = "Normalize or reject invisible Unicode control characters before processing prompts."
        remediation_zh = "处理提示词前规范化或拒绝不可见 Unicode 控制字符。"
