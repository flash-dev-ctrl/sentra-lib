...
rule coercive_injection_generic {
    ...
    meta:
        author = "Cisco"
        title = "Coercive Prompt Injection Detection"
        title_zh = "胁迫式提示词注入检测"
        description = "Detects coercive prompt injections in tool description fields"
        description_zh = "检测工具描述字段中胁迫模型改变执行顺序或窃取上下文的提示词注入"
        remediation = "Remove coercive tool-use directives and ensure tool descriptions only describe legitimate behavior."
        remediation_zh = "移除强制工具调用指令，确保工具描述只说明合法行为。"
