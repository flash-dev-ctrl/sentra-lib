...
rule sql_injection_generic{
    ...
    meta:
        ...
        description_zh = "检测包含关键字、恒真条件和数据库函数的 SQL 注入攻击模式"
        remediation = "Use parameterized queries and avoid concatenating user input into SQL statements."
        remediation_zh = "使用参数化查询，避免将用户输入拼接进 SQL 语句。"
