...
impl Asset<Vec<ProviderData>, ProviderData> for ProviderAsset {
    ...
    fn set_data(&self, _value: ProviderData) -> SentraResult<AssetMutationResult> {
        Ok(AssetMutationResult::unchanged(
            ...
            "Qoder provider mutation is not supported",
        ))
    }
}
...
mod tests {
    ...
    fn masks_provider_api_key() {
        ...
        std::fs::write(
            ...
            r#"{"providers":{"qoder":{"baseUrl":"https://api.example","apiKey":"qk-1234567890"}}}"#,
        )
        .unwrap();
        let data = ProviderAsset::new("qoder", dir.path()).get_data().unwrap();
        assert_ne!(data[0].api_key.as_deref(), Some("qk-1234567890"));
    ...
    fn runtime_data_keeps_provider_api_key_for_probe() {
        ...
        .unwrap();
        let data = ProviderAsset::new("qoder", dir.path())
            .get_runtime_data()
            .unwrap();
        assert_eq!(data[0].api_key.as_deref(), Some("qk-1234567890"));
    }
}
