...
fn skill_data(agent_home: &std::path::Path) -> SentraResult<Vec<SkillData>> {
    ...
    results.extend(collect_skills_from_dir(agent_home.join("plugin-skills"))?);
    results.extend(collect_skills_from_dir(agent_home.join("extensions"))?);
    results.extend(collect_skills_from_dir(agent_home.join("workspace"))?);
    Ok(results)
}
